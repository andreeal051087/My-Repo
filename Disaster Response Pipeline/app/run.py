...
def tokenize(text):
    ...
    ''' 
    tokenize
    Function for applyting tokenization to each of our labels by splitting into words, reducing to the root form, lower capitalization, and trimming white spaces
    
    Input:
    text : our messages text
...
def index():
    ...
    '''
    index
    Function for building the application html page, along with the visual dashboards in it.
    
    Input: none
    
    Output:
    render_template(): rendering of a master.html page.
    '''
    
    # extract data needed for visuals
    # TODO: Below is an example - modify to extract data for your own visuals
    
    # counting number of messages by genre
    genre_counts = df.groupby('genre').count()['message']
    genre_names = list(genre_counts.index)
    
     # counting number of messages within each category
    categ_msg_counts = df.iloc[:,4:].sum(axis = 0, skipna = True).sort_values(ascending = False)
    categ_names = categ_msg_counts.index.tolist()
    # percentage of messages within each category
    perc_categ_msg=df.iloc[:,4:].sum(axis = 0, skipna = True).sort_values(ascending = False)/df.shape[0]
    
    # counting number of messages within each genre and category
...
def go():
    ...
    '''
    go
    Function for rendering message classification of messages coming from user input.
    
    Input: none
    
    Output:
    render_template(): html render of classification of the message.
...
def main():
    '''
    main function: starting point for the execution of the process by defining the host, port, and calling the .run() method
    Input: none
    Output: starts the app
